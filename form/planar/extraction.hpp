...
template <typename Point>
[[nodiscard]] tbb::concurrent_vector<PointXYZNS<double>>
extract_keypoints(const PointCloud<Point> &scan,
  ...
  tbb::concurrent_vector<PointXYZNS<double>> result;
  result.reserve(planar_indices.size());
...
struct ExtractPlanar {
  ...
  ExtractPlanar(const Params &params) : params(params) {}

  template <typename Point>
  std::vector<PointXYZNS<double>> operator()(const PointCloud<Point> &scan,
                                             size_t scan_idx) const {
    auto keypoints = extract_keypoints(scan, params, scan_idx);
    // copy to a std::vector for return
    return std::vector<PointXYZNS<double>>(keypoints.begin(), keypoints.end());
  }
};
